...
const MovieCard = ({ movie }) => {
  ...
  return (
    <div onClick={()=>{navigate(`/movies/${movie._id}`);scrollTo(0,0)}} className='flex flex-col justify-between p-3 bg-gray-800 rounded-2xl hover:-translate-y-1 transition duration-300  w-66 '>
...
<div className='flex items-center gap-1 text-sm text-gray-400 mt-1 pr-1'>
  <button onClick={()=>{navigate(`/movies/${movie._id}`)}} className='px-4 py-2 text-xs bg-primary hover:bg-primary-dull transition rounded-full font-medium cursor-pointer'>Buy Ticket</button>
